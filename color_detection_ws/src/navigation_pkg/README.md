# Navigation Package of the Color Detection Project

## Overview.
The Navigation Package in the Color Detection Project is the responsible to set the throttle in the AWS DeepRacer

## Installation.
Follow the next steps to install the Navigation Package.
...
1. Build packages in the workspace:
        ...
        cd ~/deepracer_ws/color-detection-project/color_detection_ws/ && colcon build --packages-select navigation_pkg
...
Now you are running the navigation color node.
...
| ---------- | ------------ | ----------- |
|`ctrl_pkg/servo_msg` | `ServoCtrlMsg` | Sends the angle and throttle value to move in autonomous mode. |
